					...
					{#if directory.sub_directories.length > 0 && directory.files.length === 0}
						{#each directory.sub_directories as subDirectory}
							<li class="menu-item" class:active={activeDirectory?.path === subDirectory.path}>
								...
								{#if activeDirectory && subDirectory.path === activeDirectory.path}
									<ul class="menu-section">
										{#each orderBy(subDirectory.files, ['name'], ['asc']) as file}
											<li class="menu-item">
												<a href="" on:click={onMenuItemClick(subDirectory)}
													>{cleanFileName(file.name)}</a
												>
											</li>
										{/each}
									</ul>
